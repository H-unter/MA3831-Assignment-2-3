...
\subsubsection{Sample Distribution}
...
Article sentiment is modelled using VADER (Valence Aware Dictionary and sEntiment Reasoner) \cite{vader_paper}, which has shown promising results in similar contexts \cite{heiden_lstm}. The implementation, shown in \cref{code:vader}, was adapted with reference to \cite{geeksforgeeks_vader}. Lemmatisation was considered, as done by Heiden \cite{heiden_lstm}, but was ultimately excluded. The \code{nltk} implementation reduced performance, as VADER relies on common word forms that may differ from lemmatised tokens, reducing its ability to match words to sentiments.
